...
개인키랑 공개와 서명을 이용한 신원 인증 방식은 분산 원장이라는 이해가 필요한데

분산 원장 : 복제 공유 동기화된 데이터의 대한 합의 기술

금융쪽에서는 장부를 가지고 모든 거래내역을 기록하는 방식
은행이라는 금융 기관은 장부에 거래를 의존할 수 밖에 없는 중앙 집권화된 방식

분산 원장은 이것과 반대로 거래에 참여하는 모든 참여자들이 장부를 가지고 있고 거래가 발생했을 때 해당 거래내역을 강자의 장부에 기록하는 방식
중앙에서만 가지고 있는게 아니라 모두 장부를 가지고 있다.

신원 인증 방식 분산 원장 기술로 개인키, 공개키, 서명
...
비대칭형 : 사용하는 키와 복호화할 때 사용하는 키가 다른 것
다른 사람한테 절대 공개되어서는 안되는 키(비밀키) 비밀키를 토대로 만든 공개키가 한쌍이다. 결론은 키를 2개 사용한다는 것
...
너 -> 암호문 -> 너의 개인키로 복호화 -> 원본

비밀키의 소유자가 비밀키로 데이터를 암호화하고 공개키와 함께 전달한다.
공개키와 데이터를 획득한 살마은 공개키를 이용해서 복호화가 가능하고
이렇게 위험성이 보이는데 이 방법을 사용하는 이유가 데이터의 보호 목적보다는
공개키 데이터 제공한 사람의 신원을 보장해주기 때문
암호화된 데이터가 공개키로 복호화 된다는 것이 공개키와 쌍으로 이뤄지는 비밀키에 의해서 암호화가 되었다는 뜻이기 때문에 데이터 제공자가 맞는지 확인해줘서 신원이 확인 안되는 뜻
