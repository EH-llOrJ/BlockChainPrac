...
// 부모 속성 가져오고 IBlock 인터페이스 형태 클래스 만듦
...
export class Block extends BlockHeader implements IBlock {
  ...
  // 블록의 내용 _data
  // 10번째 전 Block 클래스 타입인 __adjustmentBlock
  constructor(_previousBlock: Block, _data: string[], _adjustmentBlock: Block) {
    // 부모 클래스 속성을 가져와야 하니까 super 사용
    super(_previousBlock);
    // 테스트 쪽에서 생성할 때 블록의 문자열 데이터를 _data이 데이터를 getMerkleRoot함수 안에서 _data 블록의 n번째(문자열) 머클트리로 뽑고 루트값을 반환
    this.merkleRoot = Block.getMerkleRoot(_data);
    // 블록의 해시는 블록의 내용 version, timestamp, height, merkleRoot, previousHash, difficulty, nonce
    // 가지고 해싱해서 반환 this 생성된 자신 new Block 동적할당된 블록 인스턴스
    this.hash = Block.createBlockHash(this);
    // nonce 값을 마이닝에서 0의 갯수를 난이도에 맞춰서 찾을 떄까지 증가시킬 값
    // 가장 많이 사용하는 해시 알고리즘인 SHA256으로 임의의 데이터를 해시한 값을 구해서
    // 256bit(32바이트)의 값을 얻는 것
    // 해시함수는 단방향 함수로 데이터로부터 해시값을 구할 수는 있는데
    // 해시값으로 데이터를 역산 할 수는 없다.
    // 해시값이 같은 임의의 데이터를 만들어 내는 건 정말 어렵고 (경우의 수를 다 찾는데)
    // 특정한 해시값이 나오는 데이터를 찾으려면 2256개(256비트로 나올 수 있는 모든 경우의 수)의
    // 데이터를 다 확인하는데 지금 우리 컴퓨터로는 불가능한 직업이다.

    // 해시캐시 문제는 이 불가능한 역산 작업과 유사하다.
    // 특정한 해시값이 나오는 데이터를 찾는 건 너무 힘드니까
    ...
    길이가 512bit 안되면 bit 수를 추가해주는데 zero padding
    0110 0001 0110 0010 0110 0011 .....0 0001 1000

    초기값 설정
    첫 번째 초기값은 실제로 결과가 될 값(변하는 도중임)
    첫 번째 초기값은 256bit로 총 8개의 32bit 값으로 이루어지게 되는데
    가장 작은 8개의 소수의 제곱근
    [2,3,5,7,11,13,17,19]의 제곱근
    이 배열을 반복문으로 돌려서 제곱근을 구하고 정수값을 제거하고 왼쪽으로 32bit shift 연산 후
    16진수로 출력하면 KISA에서 제공하는 SHA256 공식 소스 코드의 고정된 초기값
    0x6a09e667
    0xbb67ae85
    0x3c6ef372
    0xa54ff53a
    ...
    두 번째 초기값은 hash 연산에서 32bit 값을 가진 배열
    64개의 소수
    2, 3, 5, 7, 11 ... , 293, 307, 311
    ...
    */
    ...
    // 블록의 생성 시간을 조절해주는 것 블록이 생성되는 시간을 10마다 비교해서
    // 시간을 조절해주기 위해서 사용하는 것
  ...
  public static getDifficulty(
    ...
    // _adjustmentBlock 이 것은 10번째 전 블록
    ...
    // 난이도 조절 식
    // timeTaken이 차이값을 구한 시간이 블록의 생성 주기 시간 / 2보다 작으면
    // 난이도를 1 올려주고
    // timeTaken이 차이값이 블록 생성 주기 시간 *2 보다 크면 난이도를 1 내려준다
    // 이렇게 해서 블록의 난이도를 조절해서 블록 생성 주기를 조절해주는 것
  ...
  public static findBlock(generateBlock: Block) {
    ...
    while (true) {
      ...
      // hexToBinary 모듈 설치해서 사용 npm i hex-to-binary

      // 바이너리 코드로 바꿔서 hexToBinary 함수로 바꿔서 해시값을
      const binary: string = hexToBinary(hash);
      // startsWith() 함수는 대상의 문자열로 어떤 문자열로 시작하는지 체크 00000
      // 난이도가 4이면 0이 4개이거나 더 많은 값을 찾을 때까지 비교
