...
특정 계정 unlock 시켜주기
geth --datadir node --http http.addr "127.0.0.1" --http.port 9000 --http.corsdomain "*" \
--http.api "admin,eth,debug,miner,net,txpool,personal,web3" --syncmod full --networkid 7722\
...
eth.sendTransaction(txObject) // 위에서 만든 트랜잭션 객체



eth.getTransaction("0x14b...0fdd") // 트랜잭션의 해쉬값
contract=eth.contract(abi)

getTransaction에 넣으면 contractAddress 나옴
instance = contract.at("0x78a...a393d") // contractAddress 해쉬값
instance.getText.call()
입력하면 "Hello World!" 뜸 text.sol에서 가져왔기에

상태 변수를 가져오는 것과 변경하는 것은 차이가 있는데
상태 변수의 값을 바꾼다는 건 저장 공간에 값이 달라진다는 것이기 때문에
네트워크에서 저장공간은 한계가 있어서 저장 용량을 바꾸기 위해서 추가적으로 비용을 지불해야 한다.
EVM을 실행시키기 위한 비용을 지불하는 것.
상태 변수의 값을 바꾸려면 트랜잭션을 발생시켜 수수료를 지불해야 한다.

call 함수는 데이터를 불러오는 함수이고
값을 전달해서 상태 변수를 바꾸는 건 send이다.
트랜잭션을 보내서 배포하고 마이닝 해서 트랜잭션 처리하고 setText 함수 실행
...
miner.start(1)
miner.stop()
instance.setText("aaaaaaaa",{from:eth.coinbase})
miner.start(1)
miner.stop()
txpool로 확인해보고
instance.getText.call()
