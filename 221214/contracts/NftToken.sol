...
contract NftToken is ERC721Enumerable, Ownable {
  ...
  function tokenURI(
    ...
    // metadataURI : nft의 토큰 값에 매칭되는 앞부분 URI를 나타내는 상태 변수 baseURL이라고 생각하면 된다. 앞부분에 붙는 주소
  ...
  // 배열 전체 내용을 조회하는 함수
  // 솔리디티에서 배열을 getter로 전체 조회하는 건 불가능한데 요소를 하나만 return 하기 때문에
  // 배열 전체를 return 해주는 view 함수를 만들어주면 된다.
