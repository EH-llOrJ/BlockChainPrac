...
/*
블록이 생성되면 data에 트랜잭션을 담아주는데 이게 첫 번째 트랜잭션은 코인베이스 트랜잭션이라고 부르고 내용은 마이닝 했을 때 얻는 보상의 내용이 들어간다.

블록을 채굴하고 채굴한 사람이 얼마나 코인을 보상받았는지.
트랜잭션 내용 안에 객체가 표현되어있고
UnspentTransacitionOutPut(UTXO)이 공간에 잔액을 객체로 저장
암호화폐에서 아직 사용되지 않은 거래 출력 사용할 수 있게 승인된 일정량의 디지털 화폐 사용되지 않은 미사용 객체
...
UTXO에서 {주소 : 누구, 얻은 코인 : 50, 참조 트랜잭션 : Tx 0000}를 제ㅓㄱ하고
UTXO = [{주소 : 너, 코인 : 1, 참조 트랜잭션 : Tx 0001},{주소 : 누구, 코인 : 49, 참조 트랜잭션 : Tx 0001}]

"누구"라는 사용자가 "너"에게 50코인에서 1코인을 보낸다고 가정해서
input에 Tx 0000 트랜잭션을 속성값으로 전달하고 output에는 두 객체
...
UTXO 라는 공간은 트랜잭션의 내용을 구성해서 거래 내용을 기록해놓기 위함
...
트랜잭션 풀이라는 공간을 만들고 트랜잭션 객체가 생성됐을 때 만들어진 트랜잭션을
트랜잭션 풀에 담아준다. 그리고 블록체인 네트워크 상에 연결된 노드들은 트랜잭션 풀의 내용을 공유한다. 블록 마이닝 할 때 트랜잭션 풀 안에 있는 트랜잭션 객체를 사용해 새로 생성되는 블록의 data 속성 값으로 넣어준다.
그래서 필요한 게 블록이 마이닝 될 때 트랜잭션 풀을 업데이트 해주는 부분이 필요

노드들이 트랜잭션 풀의 내용을 공휴하는 부분은 트랜잭션 객체가 생겼을 때 트랜잭션 객체를
브로드캐스트 하는 방식
...
windows Subsystem for Linux 이걸 체크

윈도우 키 + S 검색 창에 설정 검색 -> 정보 -> 윈도우 버전 확인


작업관리자에서 가상화 사용으로 되어있어야 함
