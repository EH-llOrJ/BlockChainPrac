...
프론트와 메타마스크 연결
Dapp을 만드는 기초 작업 중 하나
트랜잭션을 보낼 때 사용자의 개인키를 사용해서 서명을 만드는 과정을 거쳐
개인키를 서버에 저장하는 것은 많이 위험하기 때문에 메타마스크와 같은 지갑 프로그램을 사용한다.

사용자의 개인키는 메타마스크가 주체가 되고 프론트에서 메타마스크에 요청을 주고 요청을 메타마스크가 받아서 사용자의 개인키를 이용해 서명을 만들어 트랜잭션을 발생 시킨 후 블록체인 네트워크로 요청을 보낸다.
메타마스크는 프론트에서 보낸 요청을 트랜잭션을 발생시켜 블록체인 네트워크로 보낸다는 것

요청
프론트 -> input -> 메타마스크(지갑) -> 블록체인 네트워크
메타마스크 : 사용자의 개인키를 이용해서 서명 만들고 전달 받은 데이터와 서명을 합쳐서 블록체인 네트워크로 전달

응답
블록체인 네트워크 -> 메타마스크(지갑) ->프론트

프론트 서버랑 메타마스크가 서로 요청 응답 받을 수 있게 만들어 주기만 하면 된다.

쇼핑몰을 예를 들어 상품들이 등록되어 있고 사용자가 코인이나 토큰으로 구매를 할 때
상품데이터까지 블록체인 네트워크에 저장하면 너무 비효율적이기 때문에
프론트, 백엔드 같이 구현해서 상품 데이터는 데이터베이스에 저장하고 상품 목록을
불러오고 특정 상품의 정보들을 가져올 땐 백엔드 서버에 요청해서 가져오는 방식

프론트에서 메타마슼로 바로 요청을 보내서 트랜잭션을 발생시키는 방법과
...
중요한 건 사용자의 개인키로 서명을 만드는 게 우리가 아니고 서명을 만들어주는 건 메타마스크가 된다는 것.
